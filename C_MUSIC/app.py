...
def index(user_id):
    # 使用 part3.user_inf 函数获取用户信息
    user_inf = part3.user_inf(user_id)

    # 使用 part3.make_tag_list10 函数获取用户前十个标签
    user_tag_top10 = part3.make_tag_list10(user_id)

    # 将用户前十个标签转换为 JSON 格式
    json_user_tag10 = json.dumps(user_tag_top10)

    # 使用 part3.make_music_list100 函数获取用户的前一百首歌曲和额外的数据
    music_top100, b = part3.make_music_list100(user_id)

    # 将前一百首歌曲的数据转换为 JSON 格式
    json_music_top100 = json.dumps(b)

    # 使用 part3.get_comment_inf 函数获取用户的评论信息
    comment_inf = part3.get_comment_inf(user_id)

    # 定义词云图片的 URL
    wordcloud_url_part3 = f"static/image/wordcloud_part3/wordcloud_part3_{user_id}.jpg"

    # 使用 part3.get_artist_top10 函数获取用户关注的前十位艺术家
    artist_list = part3.get_artist_top10(user_id)

    # 使用获取的数据渲染 "index.html" 模板
    return render_template("index.html",
                           ...
                           wordcould_url_part3=wordcloud_url_part3,
